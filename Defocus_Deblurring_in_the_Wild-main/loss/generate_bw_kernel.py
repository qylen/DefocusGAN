...
def bw_kernel_generator(k_size_, order_, cut_off_factor_, beta_, smooth_strength_):
    ...
    k_c=circle*normalize_scale(k_butter,1,beta_) #巴特沃斯核
    k_c_pad=cv2.copyMakeBorder(k_c, padding_gauss, padding_gauss, padding_gauss, padding_gauss, 0) 
    
    blur_k_c = cv2.GaussianBlur(k_c_pad,(k_size_gauss,k_size_gauss),sigma_gauss)#高斯核，高斯滤波器需要pad后？
...
# A,B,C=bw_kernel_generator(5, 3, 2.5, 0.1, 7)
# print(A,B,C)