...
class GemoLoss(nn.Module):
    ...
    def radius_dict(self, c_radius):
        ...
        for i in range(1,c_radius):
            radius_set.append(generate_vectorc(torch.tensor(i))) #这应该是个高斯模糊核

        weight_pos_set = []
        weight_neg_set = []
        for i in range(1, c_radius+1):
            current_conv = nn.Conv2d(1, 1, kernel_size=i * 2 + 1, bias=False)  #可以把这个模糊核用巴特沃斯核替代
        ...
        return radius_set, weight_pos_set, weight_neg_set #输出了一个高斯核，一个左图的模糊核，一个右图的模糊核
    ...
    def compute_gemo(self, blur_map, x):
        ...
        for i in range(self.radius):
            ...
            neg_mask = (blur_map + i + 1) * lneg_mask + (-blur_map - i + 1) * rneg_mask #如果COC图是直径，那么应该除以二，那么这里的COC就是半径了，如果用立体匹配思想，那么估计出的是直径图
            #p是前向，n是后向，应该用不同的模糊核
            if i == 0:
